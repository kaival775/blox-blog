        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          ...
          <div className="relative">
            <input
              ...
              className="search-field pl-10 w-full sm:w-80"
            />
        ...
        {loading ? (
          ...
          <div className="flex flex-col justify-center items-center gap-4">
            {posts.map((post) => (
              <div
                key={post._id}
                onClick={() => navigate(`/posts/post-details/${post._id}`)}
                className="bg-white w-1/2 rounded-lg shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-all duration-200 cursor-pointer group h-fit"
              >
                {/* <div className="h-32 bg-gray-50 flex items-center justify-center border-b border-gray-100 overflow-hidden">
                  {post.file ? (
                    ...
                    <img 
                      src={img} 
                      alt={post.title}
                      className="w-8 h-8 opacity-40 group-hover:opacity-60 transition-opacity duration-200"
                    />
                  )}
                </div> */}
                <div className="p-4">
                  ...
                  <p className="text-gray-600 text-sm leading-relaxed mb-3 line-clamp-2">
                    {post.desc.substring(0, 60)}{post.desc.length > 60 ? '...' : ''}
                  </p>
                  <div className="flex items-center justify-between text-xs text-gray-500">
                    <span className="bg-gray-100 px-2 py-1 rounded-full text-xs">
                      {post.category?.title || 'General'}
                    </span>
                    <span className="text-xs group-hover:text-black transition-colors">Read →</span>
                  </div>
                </div>
              </div>
            ))}
